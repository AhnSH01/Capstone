...
function sql_delete(table, field, target) {
  ...
  conn.query(sql, function (err, result) {
    if (err) throw err;
    console.log("Number of recoreds deleted: " + result.affectedRows);
  });
}
...
async function main() {
  ...
  // await sql_insert_user('test_id14', 'test_pw', 'test_adr', 'test_name', '010-1234-1234', 'male', 24);
  // await sql_insert_video(7, 'test_URL');
  ...
  // sql_delete('video', 'id', 6);
  // sql_delete('user', 'login_id', 'test_id6');

  // sql_update('video', 'user_id', 12, 'id', 7);
